...
void Codegen::generate(const automata::DFA& dfa, const lexer_file::LexFile& lexfile, std::ostream& out)
{
	...
	write_prologue(lexfile, out);
	write_tables(dfa, out);
	write_yylex(lexfile, out);
	write_epilogue(lexfile, out);
}
 ...
 * @param lexfile Parsed lexer file.
 */
void Codegen::write_prologue(const lexer_file::LexFile& lexfile, std::ostream& out)
{
	out << lexfile.verbatim_top_ << std::endl;
}
 ...
 * @param dfa Deterministic automaton.
 */
void Codegen::write_tables(const automata::DFA& dfa, std::ostream& out)
{
	...
	for (auto& [name, id] : dfa.start_states_) {
		...
		out << "#define " << name << " " << count << "\n";
		ids.push_back(id);
		base_conditions.push_back(name);
		count++;
	}

	out << "#define YYNB_CONDITIONS " << count << "\n";
	...
	out << "static int yy_table[" << nb_states << "][256] = {" << std::endl;
	...
	out << "static int yy_accept[" << nb_states << "] = { ";
	bool first3 = true;
	for (size_t i = 0; i < nb_states; i++) {
		if (!first3) {
			out << ", ";
 ...
 * @param lexfile Parsed lexer file.
 */
void Codegen::write_yylex(const lexer_file::LexFile& lexfile, std::ostream& out)
{
 ...
 * @param lexfile Parsed lexer file.
 */
void Codegen::write_epilogue(const lexer_file::LexFile& lexfile, std::ostream& out)
{
	out << lexfile.verbatim_bottom_;
}